...
class DataPipeline:
    ...
    async def run_full(
        ...
        try:
            ...
            logger.info(
                ...
                total_duration=sum(r.duration_seconds for r in self.stage_results.values()),
            )
    ...
    async def _extract(
        ...
        try:
            logger.info(
                "extract_stage_starting", symbols=symbols, date_range=f"{start_date} to {end_date}"
            )
    ...
    async def _validate(
        ...
        try:
            logger.info("validate_stage_starting", records_to_validate=len(candles))
            ...
            logger.info(
                "validate_stage_complete", valid=len(valid_records), invalid=len(invalid_records)
            )
    ...
    async def _deduplicate(
        ...
        try:
            logger.info("deduplicate_stage_starting", records_to_deduplicate=len(candles))
            ...
            logger.info(
                "deduplicate_stage_complete", kept=len(deduplicated), removed=len(duplicates)
            )
    ...
    async def _load(
        ...
        try:
            ...
            logger.info("load_stage_complete", records_loaded=len(candles))
    ...
    def get_pipeline_stats(self) -> dict[str, Any]:
        ...
        total_processed = sum(r.records_processed for r in self.stage_results.values())

        total_failed = sum(r.records_failed for r in self.stage_results.values())
