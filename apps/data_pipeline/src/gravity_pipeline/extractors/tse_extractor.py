...
class TSEExtractor(Extractor):
    ...
    def __init__(self, config: Optional[TSEExtractorConfig] = None):
        """Initialize TSE extractor"""
        self.config = config or TSEExtractorConfig()
        super().__init__(self.config)
        self.session: Optional[aiohttp.ClientSession] = None
        self.available_symbols: Optional[List[str]] = None
        ...
        try:
            ...
            async with session.get(
                ...
                self.available_symbols = [
                    item.get("symbol") for item in data.get("tickers", [])
                ]
                
                logger.info(
                    "symbols_fetched",
                    count=len(self.available_symbols)
                )
                return self.available_symbols
    ...
    async def extract(
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    symbol_candles = await self._extract_symbol(
                        symbol,
                        start_date,
                        end_date
                    )
