...
class SchemaValidator:
    ...
    def validate_tables_exist(self, expected_tables: list[str]) -> bool:
        ...
        if missing:
            logger.error("missing_tables", tables=list(missing), existing=existing_tables)
            return False

        logger.info("all_tables_exist", tables=expected_tables)
        return True

    def validate_column_types(self, table: str, expected_cols: dict[str, str]) -> bool:
        """
        ...
        try:
            ...
            for col_name, expected_type in expected_cols.items():
                if col_name not in col_map:
                    ...
                    mismatches.append(f"{col_name}: {col_map[col_name]} (expected {expected_type})")

            if mismatches:
                logger.error("column_type_mismatches", table=table, mismatches=mismatches)
                return False

            logger.info("column_types_valid", table=table, columns=len(expected_cols))
            return True

        except Exception as e:
            logger.error("schema_validation_error", table=table, error=str(e))
            return False
    ...
    def get_table_info(self, table: str) -> dict | None:
        ...
        try:
            ...
            logger.error("get_table_info_failed", table=table, error=str(e))
            return None
