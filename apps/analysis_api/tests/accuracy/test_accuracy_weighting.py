...
@pytest.mark.parametrize(
    "name,trend_conf,momentum_conf,cycle_conf,volume_conf",
    [
...
def test_scenario(
    name: str, trend_conf: float, momentum_conf: float, cycle_conf: float, volume_conf: float
):
    ...
    analysis = TechnicalAnalysisResult(
        ...
        trend_indicators=[
            create_test_indicator(
                "MA", IndicatorCategory.TREND, SignalStrength.BULLISH, trend_conf
            ),
            create_test_indicator(
                "MACD", IndicatorCategory.TREND, SignalStrength.BULLISH, trend_conf
            ),
        ],
        momentum_indicators=[
            create_test_indicator(
                "RSI", IndicatorCategory.MOMENTUM, SignalStrength.BULLISH, momentum_conf
            ),
            create_test_indicator(
                "Stoch", IndicatorCategory.MOMENTUM, SignalStrength.BULLISH, momentum_conf
            ),
        ],
        cycle_indicators=[
            create_test_indicator(
                "Sine", IndicatorCategory.CYCLE, SignalStrength.BULLISH, cycle_conf
            ),
            create_test_indicator(
                "Phase", IndicatorCategory.CYCLE, SignalStrength.BULLISH, cycle_conf
            ),
        ],
        volume_indicators=[
            create_test_indicator(
                "OBV", IndicatorCategory.VOLUME, SignalStrength.BULLISH, volume_conf
            ),
        ],
    )
    ...
    print("  Categories with higher accuracy have more weight in final decision")
    """Test a specific scenario"""
    print(f"\n{'=' * 60}")
    print(f"سناریو: {name}")
    print(f"{'=' * 60}")
