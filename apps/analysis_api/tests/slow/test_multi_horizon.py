...
from gravity_tech.ml.train_multi_horizon import (
    create_realistic_market_data,
    load_trained_model,
    train_multi_horizon_system,
)
...
def _test_analysis(result):
    ...
    candles = create_realistic_market_data(base_price=50000, candles_count=150, trend="mixed")
    ...
    extractor = MultiHorizonFeatureExtractor(lookback_period=100, horizons=[3, 7, 30])

    # Level 1 features
    features_l1 = extractor.extract_indicator_features(candles[-100:])

    # Create analyzer with trained model
    analyzer_l1 = MultiHorizonTrendAnalyzer(result["learner_indicators"])
    ...
    analyzer_l2 = MultiHorizonTrendAnalyzer(result["learner_dimensions"])
    ...
    return {"analysis_level1": analysis_l1, "analysis_level2": analysis_l2}
...
def test_save_and_load():
    ...
    learner = load_trained_model(
        symbol="BTCUSDT", level="indicators", model_dir="ml_models/multi_horizon"
    )
...
def generate_report(result, analyses):
    ...
    print(
        f"  30d: {analysis_l1.score_30d.score:+.3f} (conf: {analysis_l1.score_30d.confidence:.0%})"
    )
    print(
