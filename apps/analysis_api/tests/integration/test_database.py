...
class MockSQLiteDatabase:
    ...
    def __init__(self):
        self.tables = {"symbols": [], "prices": [], "analysis": [], "patterns": []}
    ...
    async def insert_symbol(self, symbol: str, name: str):
        ...
        self.tables["symbols"].append(
            {"symbol": symbol, "name": name, "created_at": datetime.now()}
        )
        return True

    async def insert_price(self, symbol: str, price_data: dict):
        """Insert price record"""
        self.tables["prices"].append({"symbol": symbol, **price_data, "created_at": datetime.now()})
        return True
...
class MockPostgresDatabase:
    ...
    async def insert_analysis(self, analysis_data: dict):
        ...
        self.tables["analysis"].append({**analysis_data, "created_at": datetime.now()})
        return True
...
class TestSQLiteDatabase:
    ...
    async def test_sqlite_get_prices(self, sqlite_db):
        ...
        for i in range(10):
            price = {"close": 100.0 + i, "volume": 1000.0}
...
class TestPostgresDatabase:
    ...
    async def test_postgres_insert_price(self, postgres_db):
        ...
        price = {"open": 100.0, "high": 110.0, "low": 90.0, "close": 105.0, "volume": 1000.0}
...
class TestBatchOperations:
    ...
    async def test_batch_insert_prices(self, sqlite_db):
        ...
        prices = [{"close": 100.0 + i, "volume": 1000.0} for i in range(100)]
    ...
    async def test_batch_insert_symbols(self, postgres_db):
        ...
        symbols = [("TEST1", "Test 1"), ("TEST2", "Test 2"), ("TEST3", "Test 3")]
...
class TestQueryOptimization:
    ...
    async def test_range_query_efficiency(self, sqlite_db):
        ...
        now = datetime.now()
        for i in range(100):
            price = {"close": 100 + i * 0.1, "timestamp": now - timedelta(days=100 - i)}
            await sqlite_db.insert_price("TEST", price)
