...
class TestMultiIndicatorConsensus:
    ...
    def test_all_indicators_agree_sell(self):
        ...
        # Simulate multiple indicators (bearish signals)
        sma20 = sum(closes[-20:]) / 20
        indicators = {
            "sma_20": closes[-1] < sma20,  # Below SMA = bearish
            "rsi": 30 < 50,  # RSI < 50 = bearish
            "macd": False,  # Negative MACD = bearish
            "bollinger": closes[-1] < sma20,  # Below middle band = bearish
        }

        # Consensus: all bearish
        bearish_count = sum(1 for v in indicators.values() if v)
...
class TestAnalysisPerformance:
    ...
    def test_analysis_with_1000_candles(self):
        ...
        # Use the calculated values to avoid unused variable warnings
        _ = sma20, sma50
