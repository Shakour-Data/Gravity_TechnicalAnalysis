...
import numpy as np
from gravity_tech.core.domain.entities import Candle, IndicatorCategory
...
def generate_test_candles(
    count: int = 50, base_price: float = 100.0, volatility: float = 0.02
) -> list[Candle]:
...
def generate_trending_candles(
    count: int = 50, base_price: float = 100.0, trend: float = 0.01
) -> list[Candle]:
...
class TestEdgeCases:
    ...
    def test_flat_prices(self):
        ...
        base_time = datetime.now()
        candles = [
            Candle(base_time + timedelta(hours=i), 100, 100, 100, 100, 1000000) for i in range(50)
        ]
    ...
    def test_single_large_gap(self):
        ...
        base_time = datetime.now()
        candles = [
            Candle(base_time + timedelta(hours=i), 100, 101, 99, 100, 1000000) for i in range(25)
        ]
        # Add large gap
...
class TestSignalBranches:
    ...
    def test_donchian_breakout_up(self):
        ...
        candles = [
            Candle(base_time + timedelta(hours=i), 100, 101, 99, 100, 1000000) for i in range(20)
        ]
        # Add breakout above
    ...
    def test_hv_extreme_annualized(self):
        ...
        result = VolatilityIndicators.historical_volatility(
            candles[-50:], period=20, annualize=True
        )
    ...
    def test_chaikin_expansion(self):
        ...
        for i in range(30):
            price = 100
            candles.append(
                Candle(base_time + timedelta(hours=i), price, price + 1, price - 1, price, 1000000)
            )
    ...
    def test_chaikin_contraction(self):
        ...
        for i in range(30):
            ...
            candles.append(
                Candle(
                    base_time + timedelta(hours=i), price, price + 10, price - 10, price, 1000000
                )
            )
        ...
        for i in range(20):
            ...
            candles.append(
                Candle(
                    base_time + timedelta(hours=30 + i), price, price + 1, price - 1, price, 1000000
                )
            )
    ...
    def test_bb_high_signal(self):
        ...
        for i in range(10):
            candles.append(
                generate_test_candles(1, volatility=0.08, base_price=candles[-1].close)[0]
            )
    ...
    def test_hv_non_annualized_extreme_high(self):
        ...
        candles = generate_test_candles(100, volatility=0.15)
        result = VolatilityIndicators.historical_volatility(
            candles[-50:], period=20, annualize=False
        )
