...
async def get_analysis_service(container: ServiceContainer = Depends(get_container_dep)):
    """
...
async def get_cache_service(container: ServiceContainer = Depends(get_container_dep)):
...
async def analyze(
    ...
    try:
        ...
        cache_key = None
        if cache:
            cache_key = f"analysis:{request.symbol}:{request.timeframe}:{len(request.candles)}"
            cached_result = await cache.get(cache_key)
            if cached_result:
                logger.info("analysis_cache_hit", symbol=request.symbol)
                return cached_result
        
        logger.info("analysis_starting", symbol=request.symbol, candles=len(request.candles))
        ...
        logger.info("analysis_complete", symbol=request.symbol, signal=result.signal)
        return result
...
async def health_ready(container: ServiceContainer = Depends(get_container_dep)) -> dict:
    """
