...
ANALYSIS_DURATION = Histogram(
    "analysis_duration_seconds", "Analysis processing time in seconds", ["analysis_type"]
)

ANALYSIS_ERRORS = Counter(
    "analysis_errors_total", "Total number of analysis errors", ["analysis_type", "error_type"]
)
...
DB_CONNECTIONS_ACTIVE = Gauge(
    "db_connections_active", "Number of active database connections", ["db_type"]
)
...
DB_ERRORS = Counter("db_errors_total", "Total number of database errors", ["db_type", "error_type"])

# ML metrics
ML_PREDICTIONS = Counter("ml_predictions_total", "Total number of ML predictions", ["model_type"])

ML_PREDICTION_DURATION = Histogram(
    "ml_prediction_duration_seconds", "ML prediction duration in seconds", ["model_type"]
)

ML_MODEL_ACCURACY = Gauge("ml_model_accuracy", "Current ML model accuracy", ["model_type"])
...
# Service info
SERVICE_INFO = Info("service_info", "Service information", ["version", "environment"])
...
class MetricsCollector:
    ...
    def record_db_operation(
        self, db_type: str, query_type: str, duration: float, error: str | None = None
    ):
