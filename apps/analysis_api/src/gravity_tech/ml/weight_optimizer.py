...
class MLWeightOptimizer:
    ...
    def _initialize_model(self):
        ...
        if self.model_type == "random_forest":
            ...
            # Ensemble of models
            self.model = GradientBoostingRegressor(
                n_estimators=200, learning_rate=0.05, max_depth=6, subsample=0.8, random_state=42
            )
    ...
    def calculate_future_return(
        self, candles: list[Candle], current_idx: int, horizon: int = 10
    ) -> float:
        """
    ...
    def predict_weights(self, features: np.ndarray) -> dict[str, float]:
        ...
        if self.optimal_weights:
            ...
            # Default weights
            weights = {"trend": 0.30, "momentum": 0.25, "cycle": 0.25, "volume": 0.20}
        ...
        default_weights = {"trend": 0.30, "momentum": 0.25, "cycle": 0.25, "volume": 0.20}

        final_weights = {}
        for key in weights:
            final_weights[key] = weights[key] * confidence + default_weights[key] * (1 - confidence)
...
class AdaptiveWeightCalculator:
    ...
    def calculate_adaptive_weights(
        ...
        if self.use_ml and self.ml_optimizer and self.ml_optimizer.model:
            try:
                ...
                for key in ["trend", "momentum", "cycle", "volume"]:
                    final_weights[key] = ml_weights[key] * 0.7 + base_weights[key] * 0.3
        ...
        normalized_weights = {
            k: base_weights[k] / total for k in ["trend", "momentum", "cycle", "volume"]
        }
