...
class MultiHorizonWeightLearner:
    ...
    def train(self, X: pd.DataFrame, Y: pd.DataFrame, verbose: bool = True):
        """
        ...
        self._evaluate_and_extract_weights(X_train, Y_train, X_test, Y_test, verbose=verbose)
    ...
    def _calculate_confidence(self, r2: float, mae: float) -> float:
        """
    ...
    def _normalize_weights(self, feature_importances: np.ndarray) -> np.ndarray:
        """
    ...
    def get_horizon_weights(self, horizon: str) -> HorizonWeights | None:
        """
    ...
    def predict_multi_horizon(self, X: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        if self.model is None:
            if self.allow_weight_fallback:
                logger.warning(
                    "multi_horizon.predict_without_model", extra={"horizons": self.horizons}
                )
        ...
        return pd.DataFrame(predictions, columns=[f"pred_{h}" for h in self.horizons])
    ...
    def _predict_with_weights(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        def _clean_value(name: str, val: float | int | None) -> float:
            ...
            if name.endswith("_percentile"):
                ...
                name.endswith("_normalized")
                or name.endswith("_signal")
                or name.endswith("_weighted")
            ):
    ...
    def save_weights(self, filepath: str):
        ...
        weights_dict = {
            ...
            "weights": {h: w.to_dict() for h, w in self.horizon_weights.items()},
        }
        ...
        for horizon, weights in self.horizon_weights.items():
            summary["horizon_details"][horizon] = {
                ...
                "top_3_features": sorted(
                    weights.weights.items(), key=lambda x: abs(x[1]), reverse=True
                )[:3],
            }
