...
class FibonacciResult:
    ...
    def retracement_levels(self) -> dict[str, float]:
        ...
        return {
            str(level.ratio): float(level.price)
            for level in self.levels
            if level.level_type == "RETRACEMENT"
        }
    ...
    def get_nearest_level(self, current_price: float) -> dict[str, Any] | None:
        ...
        nearest_ratio = min(all_levels.keys(), key=lambda r: abs(all_levels[r] - current_price))
    ...
    def get_confluence_zones(self, tolerance: float = 0.01) -> list[dict[str, Any]]:
        ...
        all_levels = [
            {
                "ratio": str(level.ratio),
                "price": float(level.price),
                "type": level.level_type.lower(),
            }
            for level in self.levels
        ]
        ...
        if len(current_zone) > 1:
            zone_avg = sum(lvl["price"] for lvl in current_zone) / len(current_zone)
            zones.append({"price": zone_avg, "levels": current_zone, "strength": len(current_zone)})
