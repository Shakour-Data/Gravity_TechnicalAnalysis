...
class ServiceContainer:
    ...
    def register(
        self, name: str, factory: Callable[["ServiceContainer"], Any], singleton: bool = False
    ) -> None:
        """
...
_container_instance: ServiceContainer | None = None
...
def _setup_container(container: ServiceContainer) -> None:
    ...
    # Register cache
    container.register("cache", create_cache_service, singleton=True)

    # Register database
    container.register("database", create_database_service, singleton=True)

    # Register analysis service
    container.register("analysis_service", create_analysis_service, singleton=True)

    # Register tool recommendation service
    container.register(
        "tool_recommendation_service", create_tool_recommendation_service, singleton=True
    )

    # Register data ingestor
    container.register("data_ingestor", create_data_ingestor_service, singleton=True)

    # Register event publisher (optional)
    if settings.features.kafka_enabled or settings.features.rabbitmq_enabled:
        container.register("event_publisher", create_event_publisher, singleton=True)
