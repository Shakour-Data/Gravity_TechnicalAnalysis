...
class DataFetcher:
    ...
    def fetch_company_price_history(company, start_date, end_date):
        ...
        try:
            ...
            if (
                "AdjFinal" not in df.columns
                and "AdjClose" in df.columns
                and "Close" in df.columns
                and "Final" in df.columns
            ):
    ...
    def fetch_sector_index_history(sector, start_date, end_date):
        ...
        try:
            ...
            for r in records:
                ...
                close_val = DataFetcher._to_float(
                    r.get("AdjFinal", r.get("AdjClose", r.get("Close", 0)))
                )
                normalized_records.append(
                    {
                        "Date": date_value,
    ...
    def fetch_all_market_indices_to_json(json_path="data/market_indices.json"):
        ...
        for func, code, name_fa in index_functions:
            ...
            try:
                ...
                try:
                    if (
                        jdatetime.datetime.strptime(start_date_jalali, "%Y-%m-%d").date()
                        > today_jalali
                    ):
                        continue
                except Exception:
        ...
        for i, sector in enumerate(sectors):
            ...
            if records:
                ...
                if not df.empty:
                    ...
                    init_price_data.insert_sector_indices(
                        sector["SectorCode"], sector["SectorName"], df
                    )
                    try:
                        max_date = pd.to_datetime(df["Date"]).max()
                        if pd.notnull(max_date):
