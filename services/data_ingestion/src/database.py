...
class init_price_data:
    ...
    def insert_price_data(records):
        ...
        for record in records:
            if isinstance(record, dict):
                ...
                adj_volume = (
                    (value / adj_final)
                    if adj_final and value
                    else (record.get("Adj Volume") or record.get("AdjVolume"))
                )
    ...
    def insert_companies(companies):
        ...
        for company in companies:
            ...
            if sector_code is not None and sector_code not in valid_sector_ids:
                # Try to find a name
                name = (
                    company.get("IndustryGroupName")
                    or company.get("IndustryName")
                    or f"Sector {sector_code}"
                )
                if sector_code not in new_sectors:
                    new_sectors[sector_code] = name
                elif new_sectors[sector_code].startswith("Sector ") and not name.startswith(
                    "Sector "
                ):
        ...
        for company in companies:
            ...
            if panel_id not in valid_panel_ids:
                if panel_id is not None:
                    missing_panels.add(
                        str(company.get("PanelID") or company.get("PanelCode") or panel_id)
                    )
                panel_id = None
        ...
        if missing_sectors:
            print(
                f"Warning: {len(missing_sectors)} sector codes not present in sectors table: {sorted(missing_sectors)}"
            )
        if missing_panels:
            print(
                f"Warning: {len(missing_panels)} panel ids not present in panels table: {sorted(missing_panels)}"
            )
        if missing_markets:
            print(
                f"Warning: {len(missing_markets)} market ids not present in markets table: {sorted(missing_markets)}"
            )
    ...
    def insert_markets(markets):
        ...
        for market in markets:
            ...
            """,
                (
                    int(market.get("MarketID")) if market.get("MarketID") is not None else None,
    ...
    def insert_panels(panels):
        ...
        for panel in panels:
            ...
            """,
                (
                    int(panel.get("PanelID")) if panel.get("PanelID") is not None else None,
                    panel.get("PanelName"),
                ),
            )
