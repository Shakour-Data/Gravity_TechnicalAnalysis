...
class TestDataFetcherExt(unittest.TestCase):
    ...
    @patch(
        "src.fetcher.pd.DataFrame.reset_index", return_value=pd.DataFrame({"Date": ["2023-01-01"]})
    )
    ...
    @patch(
        "src.fetcher.DataFetcher.fetch_company_price_history", return_value=[{"Date": "2023-01-02"}]
    )
    @patch("src.fetcher.DataFetcher.load_json", return_value=[{"Ticker": "TEST", "CompanyID": 1}])
    @patch(
        "src.fetcher.jdatetime.datetime.now",
        return_value=MagicMock(strftime=lambda fmt: "1401-10-12"),
    )
    def test_fetch_all_prices_to_json(
        self,
        mock_now,
        mock_load_json,
        mock_fetch,
    ...
    @patch(
        "src.fetcher.DataFetcher.fetch_sector_index_history",
        return_value=[{"J-Date": "1401-10-11", "Open": 100}],
    )
    @patch(
        "src.fetcher.DataFetcher.load_json",
        return_value=[{"SectorName": "فلزات اساسی", "SectorCode": 27}],
    )
    @patch(
        "src.fetcher.jdatetime.datetime.now",
        return_value=MagicMock(strftime=lambda fmt: "1401-10-12"),
    )
    def test_fetch_all_sector_indices_to_json(
        self, mock_now, mock_load_json, mock_fetch, mock_insert
    ):
