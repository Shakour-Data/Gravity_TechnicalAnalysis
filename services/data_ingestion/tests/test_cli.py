...
class TestCLI(unittest.TestCase):
    def setUp(self):
        ...
        cur.execute(
            """CREATE TABLE sectors (sector_id INTEGER PRIMARY KEY, sector_name TEXT UNIQUE)"""
        )
        cur.execute(
    ...
    def test_list_and_get_price_data_output(self):
        ...
        cur.execute(
            ...
            ("2023-01-01", 100.0, 200.0, "T1", "C1", 1),
        )
        conn.commit()
        conn.close()
    ...
    def test_main_load_market_and_sector_indices(self):
        ...
        init_price_data.insert_market_indices = staticmethod(
            lambda code, name, df: called_market.update({"count": called_market["count"] + 1})
        )
        ...
        init_price_data.insert_sector_indices = staticmethod(
            lambda code, name, df: called_sector.update({"count": called_sector["count"] + 1})
        )
    ...
    def test_main_various_commands(self):
        ...
        orig_insert_companies = init_price_data.insert_companies
        init_price_data.insert_companies = staticmethod(
            lambda d: called_update_table.update({"data": d})
        )
        try:
