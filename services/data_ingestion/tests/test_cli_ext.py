...
class TestCLIExt(unittest.TestCase):
    ...
    def test_list_sectors_output(self):
        conn = sqlite3.connect(self.test_db)
        cur = conn.cursor()
        cur.execute('INSERT INTO sectors (sector_id, sector_name) VALUES (?, ?)', (1, 'فلزات اساسی'))
        conn.commit()
        conn.close()
        f = io.StringIO()
    ...
    def test_list_companies_output(self):
        ...
        cur = conn.cursor()
        cur.execute('INSERT INTO sectors (sector_id, sector_name) VALUES (?, ?)', (1, 'فلزات اساسی'))
        cur.execute('INSERT INTO companies (company_id, ticker, name, sector_id) VALUES (?, ?, ?, ?)', ('C1', 'T1', 'شرکت تست', 1))
        conn.commit()
        conn.close()
        args = type('A', (), {})()
        args.sector_id = 1
    ...
    def test_get_price_data_output(self):
        conn = sqlite3.connect(self.test_db)
        cur = conn.cursor()
        cur.execute('INSERT INTO price_data (date, adj_close, adj_volume, ticker, company_id, sector_id) VALUES (?, ?, ?, ?, ?, ?)', ('2023-01-01', 100, 1000, 'T1', 'C1', 1))
        conn.commit()
        conn.close()
        args = type('A', (), {})()
        args.ticker = 'T1'
