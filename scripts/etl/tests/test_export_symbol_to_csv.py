...
def test_export_symbol_data_creates_dir_and_summary(monkeypatch):
    ...
    class DummyConn:
        ...
        def escape_identifier(self, s): print("escape_identifier called"); return '"' + str(s).replace('"', '""') + '"'
        def _escape_identifier(self, s): return '"' + str(s).replace('"', '""') + '"'
        def _escape_literal(self, s): return "'" + str(s).replace("'", "''") + "'"
        encoding = 'utf-8'
    class DummyEngine:
        pass
    monkeypatch.setattr(export_mod.psycopg2, "connect", lambda *a, **k: DummyConn())
...
def test_export_symbol_data_creates_dir_and_summary(monkeypatch):
    ...
    class DummyEngine:
            monkeypatch.setattr(export_mod.psycopg2, "connect", lambda *a, **k: DummyConn())
    monkeypatch.setattr(export_mod, "create_engine", lambda *a, **k: DummyEngine())
    # Patch pd.read_sql_query to return a non-empty DataFrame
    monkeypatch.setattr(pd, 'read_sql_query', lambda *a, **k: (print("read_sql_query called"), dummy_df)[1])
...
def test_main_with_symbol(monkeypatch):
    ...
    monkeypatch.setattr(export_mod, "argparse", type("MockArgparse", (), {"ArgumentParser": MockParser}))
    # Simulate main
    args = MockArgs()
    if args.symbol:
