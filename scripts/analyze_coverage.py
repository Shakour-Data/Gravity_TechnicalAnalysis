...
class CoverageAnalyzer:
    ...
    def print_recommendations(self, gaps: dict):
        ...
        if indicators:
            ...
            print("      Tests needed: SMA, EMA, RSI, MACD, Bollinger Bands, ATR")
            print("      Estimate: ~200 tests")

        if patterns:
            print(f"\n   2. Add Tests for Pattern Detection ({len(patterns)} modules)")
            print(f"      Files: {', '.join([f.split('/')[-1] for f in patterns[:3]])}")
            print("      Tests needed: Harmonic, Candlestick, Classical patterns")
            print("      Estimate: ~150 tests")

        if services:
            print(f"\n   3. Add Tests for Services ({len(services)} modules)")
            print(f"      Files: {', '.join([f.split('/')[-1] for f in services[:3]])}")
            print("      Tests needed: Analysis, Tool Recommendation, Pattern services")
            print("      Estimate: ~100 tests")

        if api:
            print(f"\n   4. Add Tests for API Endpoints ({len(api)} modules)")
            print(f"      Files: {', '.join([f.split('/')[-1] for f in api[:3]])}")
            print("      Tests needed: All v1 endpoints with different scenarios")
            print("      Estimate: ~80 tests")
