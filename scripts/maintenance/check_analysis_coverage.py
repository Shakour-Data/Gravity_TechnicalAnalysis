...
def check_analysis_coverage():
    ...
    for symbol in symbols:
        cursor.execute(
            """SELECT COUNT(DISTINCT date(analysis_date)) FROM analysis_results WHERE symbol = ? AND date(analysis_date) >= ?""",
...
def check_missing_days(symbol):
    cursor.execute(
        """SELECT DISTINCT date(analysis_date) FROM analysis_results WHERE symbol = ? AND date(analysis_date) >= ? ORDER BY date(analysis_date)""",
        (symbol, start_date.isoformat()),
    )
    days = {row[0] for row in cursor.fetchall()}
    expected_days = {(start_date + timedelta(days=i)).isoformat() for i in range(DAYS)}
