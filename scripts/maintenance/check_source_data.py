...
cursor.execute(
    "SELECT ticker, COUNT(*) FROM price_data GROUP BY ticker ORDER BY COUNT(*) DESC LIMIT 10"
)
...
cursor.execute(
    'SELECT ticker, MIN(date), MAX(date), COUNT(*) FROM price_data WHERE ticker = "وسپه" GROUP BY ticker'
)
...
cursor.execute(
    'SELECT date, timestamp FROM price_data WHERE ticker = "وسپه" ORDER BY date DESC LIMIT 5'
)
results = cursor.fetchall()
