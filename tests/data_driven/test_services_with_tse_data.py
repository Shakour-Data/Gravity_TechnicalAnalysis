...
def generate_sample_tse_candles(symbol: str = "TOTAL", count: int = 200) -> list[Candle]:
    """Generate sample TSE-like market data for testing"""
    candles = []
    base_time = datetime(2025, 1, 1)
    base_price = 1000.0  # Typical TSE price level
    volume_base = 1000000
...
def tse_db_connection():
    """TSE database connection fixture - now uses sample data"""
    # Instead of connecting to external DB, we'll use sample data
    # This avoids skips when DB is not available
    pass


@pytest.fixture
def real_tse_candles() -> list[Candle]:
    """Load real TSE market data - now generates sample data"""
    return generate_sample_tse_candles("TOTAL", count=200)


@pytest.fixture
def multiple_tse_symbols() -> dict[str, list[Candle]]:
    """Load multiple TSE symbols - now generates sample data"""
    symbols = ["TOTAL", "IRANINOIL", "PETROFF"]  # Common TSE symbols
    data = {}

    for symbol in symbols:
        data[symbol] = generate_sample_tse_candles(symbol, count=100)
...
class TestAnalysisServiceWithTSEData:
    ...
    def test_analysis_service_multiple_indicators(self, real_tse_candles):
        ...
        print("✓ All indicators computed on TSE data")
...
class TestCacheServiceWithTSEData:
    ...
    def test_cache_analysis_results(self, real_tse_candles):
        ...
        print("✓ Cached analysis result for TOTAL")
...
class TestToolRecommendationWithTSEData:
    ...
    def test_recommend_tools_based_on_tse_trend(self, real_tse_candles):
        ...
        assert len(recommended) > 0
        print("✓ Recommended tools for TSE trend")
...
class TestServiceIntegrationWithTSEData:
    ...
    def test_full_analysis_pipeline(self, real_tse_candles, service_config):
        ...
        print("✓ Full analysis pipeline completed on TSE data")
...
class TestTSEDataQuality:
    ...
    def test_tse_price_validity(self, real_tse_candles):
        ...
        print("✓ All TSE prices are valid")
